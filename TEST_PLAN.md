...
| A-05 | Hidden routes not in menu | Inspect the menu contents | No links to `/scripts` or `/hall-of-fame` appear anywhere in the UI | - [ ] |
...
| F-05 | Hall of Fame placeholder | Navigate directly to `/hall-of-fame` | Placeholder text "Find old records here!" renders with the header; no crash. (Known placeholder — content TBD) | - [ ] |
...
- Typos in visible copy on the About page ("Californiaare," "recognoized," "maintaing") and in the Standards fine print ("rur WSO," "record standar," "anually").
- Leftover `console.log` debug output in the browser console when viewing top athletes.
- `/hall-of-fame` is an unfinished placeholder (covered by F-05).
